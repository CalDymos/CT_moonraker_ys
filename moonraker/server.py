...
class Server:
    ...
    async def _initialize_component(self, name: str, component: Any) -> None:
        ...
        try:
            ...
            if e.__str__() == """[Errno 2] No such file or directory: '/mnt/UDISK/.crealityprint/upload/upload'""":
                pass
            else:
...
def main(from_package: bool = True) -> None:
    ...
    parser.add_argument(
        ...
        "-d", "--datapath", default="/mnt/UDISK/.moonraker_database",
        metavar='<data path>',
        help="Location of Moonraker Data File Path"
    )
    parser.add_argument(
        # "-c", "--configfile", default=None, metavar='<configfile>',
        "-c", "--configfile", default="/mnt/UDISK/printer_config/moonraker.conf", metavar='<configfile>',
        help="Location of moonraker configuration file")
    parser.add_argument(
        "-l", "--logfile", default="/mnt/UDISK/printer_logs/moonraker.log", metavar='<logfile>',
