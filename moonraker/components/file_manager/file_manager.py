...
class FileManager:
    ...
    def register_directory(self,
        ...
        if path != self.file_paths.get(root, ""):
            try:
                MOONRAKER_INDEX = self.server.config.get(
                    'klippy_uds_address', "/tmp/klippy_uds")
                if MOONRAKER_INDEX == "/tmp/klippy_uds":
                    config_path = "/mnt/UDISK/printer_config"
                    logs_path = "/mnt/UDISK/printer_logs"
                elif MOONRAKER_INDEX == "/tmp/klippy_uds2":
                    config_path = "/mnt/UDISK/printer_config2"
                    logs_path = "/mnt/UDISK/printer_logs2"
                elif MOONRAKER_INDEX == "/tmp/klippy_uds3":
                    config_path = "/mnt/UDISK/printer_config3"
                    logs_path = "/mnt/UDISK/printer_logs3"
                elif MOONRAKER_INDEX  == "/tmp/klippy_uds4":
                    config_path = "/mnt/UDISK/printer_config4"
                    logs_path = "/mnt/UDISK/printer_logs4"
                else:
                    config_path = "/mnt/UDISK/printer_config"
                    logs_path = "/mnt/UDISK/printer_logs"
            ...
            cx_moonraker_conf = {
                ...
                "gcodes": "/mnt/UDISK/.crealityprint/upload",
                "timelapse": "/mnt/UDISK/.crealityprint/video",
                "timelapse_frames": "/mnt/UDISK/.crealityprint/snapshot",
                "config_examples": "/usr/share/klipper/config",
                "docs": "/usr/share/klipper/docs"
            }

            if cx_moonraker_conf.get(root) and path != cx_moonraker_conf.get(root):
                path = cx_moonraker_conf.get(root)
...
class InotifyObserver(BaseFileSystemObserver):
    ...
    def add_watch(self, node: InotifyNode) -> int:
        ...
        if watch in self.watched_nodes:
            ...
            if "/mnt/UDISK/.crealityprint/upload/upload" == dir_path and os.path.islink(dir_path):
                logging.info(f"rm {dir_path}")
...
class MetadataStorage:
    ...
    async def _run_extract_metadata(self,
        ...
        if self.enable_object_proc:
            ...
            cmd += " --check-objects"
        # import os
        tmp_path = "/tmp/which_moonraker_upload"
        try:
            if os.path.exists(tmp_path):
                with open(tmp_path, "r") as f:
                    line = f.readline().strip("\n")
                    if line != self.server.klippy_connection.printer_id:
                        logging.info("which_moonraker_upload:%s printer_id:%s" % (line, self.server.klippy_connection.printer_id))
                        return
        except Exception as err:
            pass
