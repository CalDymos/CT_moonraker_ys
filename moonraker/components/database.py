...
class MoonrakerDatabase:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        # db_path = pathlib.Path(app_args["data_path"]).joinpath(".moonraker_database")
        db_path = pathlib.Path("/mnt/UDISK").joinpath(".moonraker_database")
        if (
            ...
            dep_path = dep_path or "/mnt/UDISK/.moonraker_database"
        ...
        logging.error("##########%s" % self.database_path)
        import os
        if os.path.exists("/etc/init.d/klipper_service.2") and self.server.klippy_connection.printer_id != "1":
            self.database_path = os.path.expanduser(config.get(
                'database_path', "/mnt/UDISK/.moonraker_database" + "." + self.server.klippy_connection.printer_id))
