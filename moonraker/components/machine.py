...
class Machine:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.server.register_endpoint(
            "/machine/get_machine_info", ['GET'],
            self._handle_get_machine_info_request)
    ...
    async def _handle_multi_printer_info_request(self,
        ...
        :return:
        """
        from .printer_list import PRINTER_LIST
        def get_host_ip():
            """
            查询本机ip地址
            :return: ip
    ...
    async def _handle_get_machine_info_request(self,
                                               web_request: WebRequest
                                               ) -> Dict[str, Any]:
        """获取本机信息"""
        ...
        try:
            ...
            df_cmd = """df |grep /mnt/UDISK"""
            udisk_free_size = check_output(df_cmd, shell=True).decode('utf-8')
            udisk_free_size = udisk_free_size.strip().split()[2]
            udisk_free_size = (float(udisk_free_size)) / 1024
            if udisk_free_size > 1024:
                local_size = f"{round(udisk_free_size / 1024, 1)}G"
            elif udisk_free_size <= 0:
                local_size = "0M"
            else:
                local_size = f"{round(udisk_free_size, 1)}M"
            total_size = "8.0G"
            cmd = "cat /proc/cmdline"

            def get_serial_number(cmd):
                serial_number = "unknown"
                try:
                    ret = check_output(cmd, shell=True).decode().split(" ")
                    for obj in ret:
                        if obj.startswith("dn="):
                            if obj.split("=")[1]:
                                serial_number = obj.split("=")[1]
                                break
                        elif obj.startswith("androidboot.serialno"):
                            serial_number = obj.split("=")[1]
                except Exception as err:
                    pass
                return serial_number

            serial_number = get_serial_number(cmd)
            ...
            def get_yaml_info(_config_file=None):
                ...
                with open(_config_file, 'r', encoding='utf-8') as f:
                    config_data = yaml.load(f.read(), Loader=yaml.Loader)
                return config_data
            printer_id = get_yaml_info("/mnt/UDISK/.crealityprint/default_printer.yaml")
            str_printer_id = str(printer_id)

            def get_printer_name(printer_id):
                try:
                    printer_path = f"/mnt/UDISK/.crealityprint/printer_{printer_id}.yaml"
                    printer_info = get_yaml_info(printer_path)
                    if printer_info:
                        return printer_info.get("name", "unknown")
                except:
                    pass
                return "unknown"
            if str_printer_id != "1":
                printer_cfg_path = os.path.join("/mnt/UDISK/printer_config" + str_printer_id, "printer.cfg")
                printer_name = get_printer_name(printer_id)
            # t12 = time.time()
            def get_printer_size_by_notes(path):
                """
                获取打印机尺寸
                path = 'printer.cfg'
                """
                try:
                    # 获取printer_size注释内容
                    printer_size_cmd = "cat %s |grep printer_size" % path
                    logging.info(printer_size_cmd)
                    ret = check_output(printer_size_cmd, shell=True)
                    printer_size = ret.decode("utf8").split(":")[1]
                    return printer_size
                except Exception as e:
                    logging.info(e)
                    return None
            machine_size = get_printer_size_by_notes(printer_cfg_path)
            # t13 = time.time()
            # t14 = time.time()
            def get_printer_size(path):
                """
                获取打印机尺寸
                path = 'printer.cfg'
                """
                try:
                    ...
                    try:
                        ...
                        cp.read(path, encoding="gbk")
                    x = cp.get('stepper_x', 'position_max')
                    y = cp.get('stepper_y', 'position_max')
                    z = cp.get('stepper_z', 'position_max')
                    result = f"{x}x{y}x{z}"
                    return result
                except Exception as e:
                    logging.exception(e)
                    return None
            machine_size = machine_size if machine_size else get_printer_size(printer_cfg_path)
            # t15 = time.time()
            machine_path = "/mnt/UDISK/.crealityprint/machine.yaml"
            # t16 = time.time()
            machine_name = get_yaml_info(machine_path)

            def _get_local_system_version():
                """获取本地系统版本"""
                get_local_system_cmd = "fw_printenv -n swu_version"
                try:
                    result = check_output(get_local_system_cmd,
                    ...
                    logging.exception(e)
                    return None
            version = "V" + _get_local_system_version()
            # t19 = time.time()
            # 获取软件编译时间
            # cat /etc/openwrt_release
            get_complie_time_cmd = "cat /etc/openwrt_release | grep 'DISTRIB_ID'"
            try:
                result = check_output(get_complie_time_cmd, shell=True).decode('utf-8').replace("\n", "")
                complie_time_str = result.split('.')[-2]
                complie_time = complie_time_str[0:4] + " " + complie_time_str[4:6] + " " + complie_time_str[6:8]
            except Exception as e:
                logging.exception(e)
                complie_time = "-"
            # t20 = time.time()
            try:
                mac = check_output("ip addr", shell=True).decode().upper()
            except Exception as err:
                mac = ""
            ...
            for line in lines:
                if "ETH0:" in line:
                    ...
                    if eth0_flag:
                        ...
                        mac_addr_wifi = line.strip().split()[1]
                        wlan0_flag = False
            # 获取累计时间, 单位秒
            def get_now_time():
                """获取当前系统启动时长"""
            ...
            def seconds_to_dhms(seconds):
                def _days(day):
                    return "{}d".format(day) if day > 1 else "{}d".format(day)

                def _hours(hour):
                    return "{}h".format(hour) if hour > 1 else "{}h".format(
                        hour)

                def _minutes(minute):
                    return "{}m".format(minute) if minute > 1 else "{}m".format(
                        minute)

                def _seconds(second):
                    return "{}s".format(second) if second > 1 else "{}s".format(
                        second)
            ...
            # 转化天、时、分
            now_time_str = seconds_to_dhms(now_time)

            # t24 = time.time()
            # 转化为 00:00:00 天时分
            # t25 = time.time()
            def minutes_to_simple_dhm(minutes):
                def _hours(hour):
                    return "%02d" % hour
                ...
                hours = minutes // 60
                if hours > 0:
                    return _hours(hours) + ":" + _minutes(minutes % 60)
                if minutes > 0:
                    return "00:" + _minutes(minutes)
                return "00:00"

            def get_total_use_minutes():
                """返回累计使用分钟数"""
                return get_yaml_info(
                    "/mnt/UDISK/.crealityprint/total_time.yaml").get(
                    "total_time", 0)
            total_minutes = int(get_total_use_minutes())
            up_right_now_time_str = minutes_to_simple_dhm(total_minutes)
            ...
            def get_printer_model(path):
                ...
                try:
                    ...
                    logging.exception(e)
                    return None
            printer_type = get_printer_model(printer_cfg_path)
            printer_type = printer_type if printer_type else "unknown"
            ...
            def get_printer_png(printer_model=None, path=None):
                """获取机型图片"""
                ip_addr = get_host_ip()
                pre_addr = "http://{ip_addr}:9005/".format(
                    ip_addr=ip_addr)
                printer_image_path = pre_addr + "klipper-brain_printer_config/png/other.png"
                if printer_model == "Ender-3 Laser":
                    return pre_addr + "klipper-brain_printer_config/png/Ender3-Laser.png"
                elif printer_model == "Ender-5 S1":
                    try:
                        import configparser
                        c = configparser.ConfigParser(strict=False)
                        c.read(path)
                        x_offset = float(c.get("bltouch", "x_offset"))
                        if x_offset < -1:
                            return pre_addr + "klipper-brain_printer_config/png/Ender-5s1_2.png"
                    except Exception as e:
                        logging.exception(e)
                if printer_model:
                    from .printer_list import PRINTER_LIST
                        ...
                        for v1 in v.values():
                            for v2 in v1:
                                ...
                                if printer_model == v2.get("printer_model"):
                                    return pre_addr + v2.get("image")
                return printer_image_path
            printer_image_path = get_printer_png(printer_type, printer_cfg_path)

            def get_chip_type(path):
                try:
                    chip_type_list = ["STM32F103", "STM32F401"]
                    get_chip_cmd1 = f'cat {path} | grep "with a"'
                    get_chip_cmd2 = f'cat {path} | grep "select the"'
                    ret = check_output(get_chip_cmd1,
                                       shell=True).decode().split(" ")
                    if ret[1] == "with":
                        ret2 = check_output(get_chip_cmd2,
                                            shell=True).decode().split(" ")
                        result = ret2[-1].strip()
                    else:
                        result = ret[1].strip()
                    if result in chip_type_list:
                        return result
                except Exception as e:
                    logging.info("not chip type info")
                return ""
            chip_type = get_chip_type(printer_cfg_path)

            def get_shapre_calibrate_type(machine_type):
                """
                获取振动补偿方式
                返回 0:先测X 再测Y 1: XY同时测 2:用户自定义选择
                """
                relax = """Ender-3 S1,0
            Ender-3 S1 Pro,0
            Ender-3 V2,0
            ...
            Sermoon D1,1"""
                try:
                    ...
                    pass
                return 2
            shapre_calibrate_type = get_shapre_calibrate_type(printer_type)
            # t32 = time.time()
            data = {
                "mac_addr_wifi": mac_addr_wifi,
                "mac_addr": mac_addr,
                "machine_name": machine_name,  # 屏幕名称
                "serial_number": serial_number,  # 屏幕序列号
                "use_time": now_time_str,  # 累计使用时间
                "up_right_now_time_str": up_right_now_time_str,  # 累计使用时间右上角使用 00:00:00 天时分
                "local_size": local_size + "/" + total_size,  # 屏幕容量 剩余空间/总空间
                "machine_version": version,  # TODO: 终端固件获取需要吴辉协助
                "printer_type": printer_type,  # 打印机型号
                "machine_size": machine_size,  # 打印尺寸
                "printer_image_path": printer_image_path,  # 打印机图片预览
                "complie_time": complie_time,  # 软件编译时间
                "chip_type": chip_type,
                "printer_name": printer_name,
                "shapre_calibrate_type": shapre_calibrate_type
            }
            return data
        except Exception as err:
            logging.exception(err)
...
class BaseProvider:
    ...
    async def shutdown(self) -> None:
        ...
        await self._exec_sudo_command(f"poweroff")
    ...
    async def _detect_active_services(self) -> None:
        ...
        try:
            ...
            for svc in services:
                ...
                for allowed in DEFAULT_ALLOWED_SERVICES:
...
class InstallValidator:
    ...
    async def _check_service_file(self) -> bool:
        ...
        if not cfg_path or not cfg_file.parent.samefile(cfg_path):
            ...
            # cfg_file = self.data_path.joinpath(f"{cfg_file.name}")
            cmd_args += f" -c {cfg_file}"
    ...
    async def _check_configuration(self) -> bool:
        ...
        try:
            ...
            default_db = pathlib.Path("/mnt/UDISK/.moonraker_database").expanduser()
