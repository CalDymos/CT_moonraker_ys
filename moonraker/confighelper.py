    ...
    def _get_option(self,
        ...
        try:
            ...
            if default is Sentinel.MISSING:
                ...
                raise ConfigError("""{"code":"key140", "msg": "No option found (%s) in section [%s]", "values": ["%s", "%s"]}""" % (option, self.section, option, self.section))
            val = default
            section = self.section
        except Exception:
            raise ConfigError(
    ...
    def _parse_file(
        ...
        try:
            ...
            if not file_path.is_file():
                raise ConfigError(
                """{"code":"key151", "msg": "Configuration File Not Found: '%s'", "values": ["%s"]}""" % (file_path, file_path)
            )
            ...
            raise ConfigError(
                              """{"code":"key152", "msg": "Error Reading Config: '%s'", "values": ["%s"]}""" % (file_path, file_path)
                              )
