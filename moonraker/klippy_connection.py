...
def upload_log_to_remote_server(message):
    ...
    #     multi_printer_info = get_yaml_info(MULTI_PRINTER_PATH)
    #     multi_printer_info_list = multi_printer_info.get("multi_printer_info")
    #     for printer_info in multi_printer_info_list:
    #         if str(printer_info.get("printer_id")) == MOONRAKER_INDEX:
    #             printer_info["status"] = 0
    #             set_yaml_info(MULTI_PRINTER_PATH, multi_printer_info)
    #             break
    urllib.request.urlopen(
        "http://127.0.0.1:8000/settings/machine_info/?method=record_log_to_remote_server&message={}&index={}".format(
            message, MOONRAKER_INDEX))

def thread_upload_log_to_remote_server(message):
    import threading
    t = threading.Thread(target=upload_log_to_remote_server, args=(message, ))
    t.start()
...
def deal_bed_mesh_response(response):
    ...
    if probe_count and mesh_min and mesh_max:
        ...
        if offset and z:
            ...
            if number:
                # "//{number:1, x:-0.000, y:40.00, z:1.259375}"
                x, y = offset.split(',')
                # try:
                #     import urllib.request
                #     urllib.request.urlopen(
                #             "http://127.0.0.1/settings/machine_info/?method=set_bed_mesh_default_data&number={}&z_point={}".format(
                #                 number, z))
                # except Exception as e:
                #     logging.exception(e)
                # new_response = "// {number:%s, x:%s, y:%s, z:%s}" % (number, x, y, z)
...
class KlippyConnection:
    def __init__(self, server: Server) -> None:
        ...
        self.uds_address: str = server.config.get(
            'klippy_uds_address', "/tmp/klippy_uds")
        global MOONRAKER_INDEX
        MOONRAKER_INDEX = str(self.uds_address)
    ...
    def _process_gcode_response(self, response: str) -> None:
        ...
        #                  "is {}".format(response))
        if "auto_bed_level" in response:
            response = response.replace(" auto_bed_level", "auto_bed_level")
    ...
    async def request(self, web_request: WebRequest) -> Any:
        ...
        if rpc_method == "objects/subscribe":
            ...
            if rpc_method == "gcode/script":
                ...
                # if "TESTZ" in script:
                #     try:
                #         z_offset = script.split("Z=")[-1].strip()
                #         import urllib.request
                #         urllib.request.urlopen("http://127.0.0.1/settings/machine_info/?method=set_z_offset_status&moonraker_z_offset={}".format(z_offset))
                #     except Exception as e:
                #         logging.exception(e)
                # if "FIRMWARE_RESTART" in script:
                #     import threading
                #     logging.info("+++++++++++++++{}".format(script))
                #     import urllib.request
                #     urllib.request.urlopen("http://127.0.0.1/settings/machine_info/?method=record_log_to_remote_server&message={}".format(script))
                ...
                if "FIRMWARE_RESTART" in script:
                    logging.info("+++++++++++++++{}".format(script))
                    thread_upload_log_to_remote_server("emergency_stop")
                if script:
                    self.server.send_event(
                        "klippy_connection:gcode_received", script)
            elif rpc_method.endswith("emergency_stop"):
                logging.info("+++++++++++++++++emergency_stop")
                thread_upload_log_to_remote_server("emergency_stop")
